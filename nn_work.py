...
class NeuralNetwork:
    ...
    def sigmoid(self, x):
        # computer and return sigmoid activation value for a fiven value
    ...
    def fit_partial(self, x, y):
        ...
        # reverse the deltas to reverse the deltas
        D = D[::-1]

        # WEIGHT UPDATE PHASE
        # loop over the layers
        for layer in np.arange(0, len(self.W)):
            # update out weights by tacking the dot product of the layer
...
if __name__ == '__main__':
    ...
    # now that our network is trained, loop over the Xor data
    for (x, target) in zip(X, y):
